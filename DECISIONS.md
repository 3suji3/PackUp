...
- 문제 상황: 기존 화면은 기능은 동작하지만 blue 중심 색상과 긴 설명 때문에 첫 행동이 충분히 또렷하게 보이지 않았다.
- 선택지: 기존 blue 유지 / warm orange 중심으로 재정의
- 선택 이유: PackUp은 준비를 돕는 생활형 서비스라 차가운 blue보다 warm orange가 더 가볍고 친근한 인상을 준다. rose는 저장/추가 같은 행동 유도 버튼에만 써서 사용자의 클릭 지점을 명확히 한다.
- UX 결정: Hero 문구를 "준비할 상황을 골라주세요"로 줄이고, 상황 선택 영역을 별도 강조 섹션으로 올려 "상황 선택 → 체크리스트 생성" 흐름을 바로 보이게 한다.
- 인터랙션 결정: 진행률, 완료 메시지, 체크 시 작은 scale animation만 추가한다. 모두 기존 checklist items에서 파생되는 표현이라 상태 관리와 localStorage 구조는 변경하지 않는다.
- trade-off: 색상 수를 줄인 만큼 화면의 장식성은 낮아지지만, MVP에서 중요한 행동 흐름과 저장 구조 설명 가능성이 더 좋아진다.
